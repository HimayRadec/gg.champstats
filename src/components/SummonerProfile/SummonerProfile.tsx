...
function SummonerProfileComponent(props: { accountData: AccountInformation }) {
   ...
   useEffect(() => {
      const fetchData = async () => {
         try {
            const fetchedSummonerProfileData = await fetchSummonerProfile(props.accountData.puuid);
   ...
   return (
      <div className="flex items-center gap-x-4 p-4 border">
         ...
         <div className="border-4 rounded-md w-fit">
            <Image src={`https://ddragon.leagueoflegends.com/cdn/14.13.1/img/profileicon/588.png`} alt="Summoner Icon" width={100} height={100} />
         </div>
         <div className="flex flex-col items-center gap-y-2">
            <h1 className="font-bold">
               {props.accountData.gameName}
               <span className="font-light">#{props.accountData.tagLine}</span>
            </h1>
